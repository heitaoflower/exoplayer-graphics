...
public class Matrix3x3d {
    ...
    public Matrix3x3d(final float m00, final float m01, final float m02, final float m10, final float m11, final float m12, final float m20, final float m21, final float m22) {
        super();
        (this.m = new float[9])[0] = m00;
        this.m[1] = m01;
        this.m[2] = m02;
        this.m[3] = m10;
        this.m[4] = m11;
    ...
    public void setSameDiagonal(final float d) {
        this.m[0] = d;
        this.m[4] = d;
        this.m[8] = d;
    }
    ...
    public void transpose(final Matrix3x3d result) {
        final float m1 = this.m[1];
        final float m2 = this.m[2];
        final float m3 = this.m[5];
        result.m[0] = this.m[0];
        result.m[1] = this.m[3];
        result.m[2] = this.m[6];
        result.m[3] = m1;
    ...
    public static void mult(final Matrix3x3d a, final Vector3d v, final Vector3d result) {
        final float x = a.m[0] * v.x + a.m[1] * v.y + a.m[2] * v.z;
        final float y = a.m[3] * v.x + a.m[4] * v.y + a.m[5] * v.z;
        final float z = a.m[6] * v.x + a.m[7] * v.y + a.m[8] * v.z;
        result.x = x;
        result.y = y;
        result.z = z;
    }
    ...
    public boolean invert(final Matrix3x3d result) {
        final float d = this.determinant();
        if (d == 0.0) {
            return false;
        }
        final float invdet = 1.0f / d;
    ...
    public String toString() {
        ...
        for (int i = 0; i < 9; ++i)
        {
            builder.append(Float.toString(this.m[i]));
            if (i < 9 - 1)
            {
                builder.append(", ");
            }
