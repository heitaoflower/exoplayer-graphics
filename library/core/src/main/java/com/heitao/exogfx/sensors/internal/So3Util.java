...
public class So3Util {
    private static final float M_SQRT1_2 = 0.7071067811865476f;
    private static final float ONE_6TH = 0.1666666716337204f;
    private static final float ONE_20TH = 0.1666666716337204f;
    ...
    public static void sO3FromTwoVec(final Vector3d a, final Vector3d b, final Matrix3x3d result) {
        ...
        if (So3Util.sO3FromTwoVecN.length() == 0.0) {
            final float dot = Vector3d.dot(a, b);
            if (dot >= 0.0) {
                result.setIdentity();
            }
            else {
    ...
    private static void rotationPiAboutAxis(final Vector3d v, final Matrix3x3d result) {
        ...
        So3Util.rotationPiAboutAxisTemp.scale(3.141592653589793f / So3Util.rotationPiAboutAxisTemp.length());
        final float invTheta = 0.3183098861837907f;
        final float kA = 0.0f;
        final float kB = 0.20264236728467558f;
    ...
    public static void sO3FromMu(final Vector3d w, final Matrix3x3d result) {
        final float thetaSq = Vector3d.dot(w, w);
        final float theta = (float)Math.sqrt(thetaSq);
        float kA;
        float kB;
        if (thetaSq < 1.0E-8) {
            ...
            final float invTheta = 1.0f / theta;
            kA = (float)Math.sin(theta) * invTheta;
            kB = (float)(1.0f - Math.cos(theta)) * (invTheta * invTheta);
        }
        rodriguesSo3Exp(w, kA, kB, result);
    }
    
    public static void muFromSO3(final Matrix3x3d so3, final Vector3d result) {
        final float cosAngle = (so3.get(0, 0) + so3.get(1, 1) + so3.get(2, 2) - 1.0f) * 0.5f;
        result.set((so3.get(2, 1) - so3.get(1, 2)) / 2.0f, (so3.get(0, 2) - so3.get(2, 0)) / 2.0f, (so3.get(1, 0) - so3.get(0, 1)) / 2.0f);
        final float sinAngleAbs = result.length();
        if (cosAngle > 0.7071067811865476) {
            if (sinAngleAbs > 0.0) {
                result.scale((float)Math.asin(sinAngleAbs) / sinAngleAbs);
            }
        }
        else if (cosAngle > -0.7071067811865476) {
            final float angle = (float)Math.acos(cosAngle);
            result.scale(angle / sinAngleAbs);
        }
        else {
            final float angle = 3.141592653589793f - (float)Math.asin(sinAngleAbs);
            final float d0 = so3.get(0, 0) - cosAngle;
            final float d = so3.get(1, 1) - cosAngle;
            final float d2 = so3.get(2, 2) - cosAngle;
            final Vector3d r2 = So3Util.muFromSO3R2;
            if (d0 * d0 > d * d && d0 * d0 > d2 * d2) {
                r2.set(d0, (so3.get(1, 0) + so3.get(0, 1)) / 2.0f, (so3.get(0, 2) + so3.get(2, 0)) / 2.0f);
            }
            else if (d * d > d2 * d2) {
                r2.set((so3.get(1, 0) + so3.get(0, 1)) / 2.0f, d, (so3.get(2, 1) + so3.get(1, 2)) / 2.0f);
            }
            else {
                r2.set((so3.get(0, 2) + so3.get(2, 0)) / 2.0f, (so3.get(2, 1) + so3.get(1, 2)) / 2.0f, d2);
            }
    ...
    private static void rodriguesSo3Exp(final Vector3d w, final float kA, final float kB, final Matrix3x3d result) {
        final float wx2 = w.x * w.x;
        final float wy2 = w.y * w.y;
        final float wz2 = w.z * w.z;
        result.set(0, 0, 1.0f - kB * (wy2 + wz2));
        result.set(1, 1, 1.0f - kB * (wx2 + wz2));
        result.set(2, 2, 1.0f - kB * (wx2 + wy2));
        float a = kA * w.z;
        float b = kB * (w.x * w.y);
        result.set(0, 1, b - a);
        result.set(1, 0, b + a);
