...
public class
HeadTracker implements SensorEventListener {
    ...
    public void getLastHeadView(final float[] headView, final int offset) {
        ...
        synchronized (this.mTracker) {
            ...
            long clockTime = this.mClock.nanoTime();
            final float secondsSinceLastGyroEvent = TimeUnit.NANOSECONDS.toSeconds(clockTime - this.mLatestGyroEventClockTimeNs);
            final float secondsToPredictForward = secondsSinceLastGyroEvent + 1.0f/30;
            final float[] mat = this.mTracker.getPredictedGLMatrix(secondsToPredictForward);
